...
def paragraph_chunks(t, min_len=200):
    ...
    out=[]; buf=""
    for x in p:
        if len(x)<min_len: buf+=" "+x
        else:
            if buf: out.append((buf+" "+x).strip()); buf=""
            else: out.append(x)
...
def agentic_answer(text, q):
    ...
    numbered="\n".join([f"{i+1}. {cand[i][:400]}" for i in range(len(cand))])
    sel=f"""
Pick most relevant chunks. Return JSON list only.
...
"""
    ...
    try:
        ...
        chosen=[cand[x-1] for x in arr[:5] if 1<=x<=len(cand)]
    except:
        chosen=[cand[0]]
...
if st.button("Get Answer"):
    ...
    with st.spinner("Thinking..."):
        ...
        # ---- Get Chunks ----
        if method=="Fixed chunking": ch=fixed_chunks(doc_text)
            ...
            with st.expander("🔍 Show Evidence (Top Selected Chunks)"):
                for i,ck in enumerate(chosen,1):
